...
(() => {
    ...
    window.addEventListener('load', () => {
        ...
        // アイコンのURL
        const iconEyeOpen = 'https://cdn-icons-png.flaticon.com/512/2767/2767194.png'; // 開いている目
        const iconEyeClose = 'https://cdn-icons-png.flaticon.com/512/2767/2767146.png'; // 閉じている目

        // 「#menu が表示されている状態」で使うアイコンは「目が開いている」想定
        // 初期状態で #menu が表示されているとみなし、openEyeIcon をセット
        let currentIcon = iconEyeOpen;

        // 既存のクラス(globaliconinner)を流用したdivを作成
        const toggleMenuIcon = document.createElement('div');
        toggleMenuIcon.id = 'toggleMenuEyeIcon';
        toggleMenuIcon.className = 'globaliconinner'; // 既存と同じクラスで見た目合わせ
        toggleMenuIcon.title = 'Toggle #menu';
        ...
        // アイコンをdivの子要素として配置
        toggleMenuIcon.appendChild(iconImg);

        // #noteicon の「すぐ後ろ」に追加
        noteIconEl.insertAdjacentElement('afterend', toggleMenuIcon);
        ...
        toggleMenuIcon.addEventListener('click', () => {
            ...
            if (menuEl.style.display === 'none') {
                ...
                currentIcon = iconEyeOpen;
            } else {
                // メニューが表示されていたので非表示にする
                menuEl.style.display = 'none';
                currentIcon = iconEyeClose;
            }
            // アイコン画像を更新
            iconImg.src = currentIcon;
        });
